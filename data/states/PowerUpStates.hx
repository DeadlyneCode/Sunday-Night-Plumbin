...
var isJesus:Bool = isPause && (PlayState.instance.curSong == "blessing");
...
function create() {
	powerUps = copyArray(PowerUpState_powerUpData);
	
	if (isPause || !(FunkinSave.getSongHighscore("Poltergeist", "normal").score > 0))
	    powerUps.pop();
	else
	...
	var powerPath:String = "states/powerup/" + (isJesus ? "roblox/" : "");
	var powerFont:String = isJesus ? "SourceSans3-Bold.ttf" : "U.ttf";

	var bg = new FunkinSprite();
	bg.loadGraphic(Paths.image(powerPath + "background"));
	...
	if (!isPause) {
    	...
    	for (p=>powerUp in powerUps) {
    		...
    		power.loadGraphic(Paths.image(powerPath + "items/" + powerUp[0]));
    	...
    	color.makeGraphic(FlxG.width, 10);
    	...
    	curMenuTxt.setFormat(Paths.font(powerFont), 26, FlxColor.WHITE);
    	...
    	curPowerTxt.setFormat(Paths.font(powerFont), 26, FlxColor.WHITE, 'right');
	...
	for (dir in ['left', 'right']) {
		...
		for (p=>pipe in [pipeBack, pipeFront]) {
		    var pipeAnim:String = ["pipeBack", "pipeFront"][p];

		    pipe.frames = Paths.getFrames(powerPath + "pipe");
			...
			pipe.y = (camPower.height - pipe.height) / 2;
		}
		...
		stupidPipeHeight = pipeFront.height;
	}
	...
	for (p=>powerUp in powerUps) {
		...
		power.antialiasing = true;
		power.ID = p;
		power.y = stupidPipeY + (stupidPipeHeight - power.height) / 2;
		insert(4, power);
	...
	powerTxt.setFormat(Paths.font(powerFont), 64, FlxColor.WHITE, "center");
	...
	powerDescTxt.setFormat(Paths.font(powerFont), 32, FlxColor.WHITE, "center");
