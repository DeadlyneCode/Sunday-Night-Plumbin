...
vec2 crt_coords(vec2 uv, float bend)
{
	uv -= 0.50; //XYfleche
    uv *= 2.3;  //Yfleche
    uv.x *= 1. + pow(abs(uv.y)/bend, 2.); //DEFORMATION
...
void mainImage()
{
    ...
    //float s1 = scanline(uv, 200., 0.);
    //float s2 = scanline(uv, 0., 0.);
    
    vec4 col;
    col.r = texture(iChannel0, crt_uv + vec2(0., 0.0)).r;
    col.g = texture(iChannel0, crt_uv + vec2(0., 0.0)).g;
    col.b = texture(iChannel0, crt_uv + vec2(0., -0.0)).b;
    col.a = texture(iChannel0, crt_uv).a;
    
    //col = mix(col, vec4(s1 + s2), 0.05);
	fragColor = mix(col, vec4(noise(uv * 75.)), 0.05) * vignette(uv, 1.0, .6, 8.);;
